...
exports.userLogin = (req, res, next) => {
    ...
    docClient.query(awsSearchByEmailParms(email), (err,data)=>{
        if(err){
            res.status(503).json({"status": "Failed","error": err});
        }else if(data.Items.length ===1 && bcrypt.compareSync(password, data.Items[0].Password)){
            var payload = {"identity": email};
            var key = fs.readFileSync('private.key');
            var token = jwt.sign(payload,key,{ algorithm: 'RS256', expiresIn:'1h'});
            jwt.verify(token, fs.readFileSync('public.key'),{ algorithms: ['RS256'] }, function(err, decoded) {
                console.log(decoded);
              });
            ...
            res.status(200).json({"status":"Failed", "jwt":null});
